...
def volume_delete(ctx: click.Context, volume_id: str, yes: bool, dry_run: bool, wait: bool) -> None:
    ...
    if dry_run:
        data = client.get(url)
        vol = data.get("volume", data)
        console.print(f"[yellow]Would delete volume:[/yellow]")
...
@volume.command("backup-list")
@click.option("--all-projects", is_flag=True, default=False, help="Include all projects (admin).")
@output_options
@click.pass_context
def volume_backup_list(ctx: click.Context, all_projects: bool,
    ...
    print_list(
        ...
        [
            ("ID", "id", {"style": "cyan", "no_wrap": True}),
            ("Name", lambda b: b.get("name", "") or "—", {"style": "bold"}),
            ("Volume ID", lambda b: b.get("volume_id", "") or "—"),
            ("Status", "status", {"style": "green"}),
            ("Size (GB)", lambda b: str(b.get("size", "")), {"justify": "right"}),
            ("Incremental", lambda b: "yes" if b.get("is_incremental") else "no"),
            ("Created At", lambda b: str(b.get("created_at", ""))[:19]),
        ],
        title="Volume Backups",
        output_format=output_format, fit_width=fit_width, max_width=max_width,
        noindent=noindent, columns=columns,
...
@output_options
@click.pass_context
def volume_backup_show(ctx: click.Context, backup_id: str,
                       output_format: str, columns: tuple[str, ...], fit_width: bool,
                       max_width: int | None, noindent: bool) -> None:
    """Show Cinder volume backup details."""
    client = ctx.find_object(OrcaContext).ensure_client()
    data = client.get(f"{client.volume_url}/backups/{backup_id}")
    bkp = data.get("backup", data)
    fields = [
        ("id", str(bkp.get("id", "") or "")),
        ("name", str(bkp.get("name", "") or "")),
        ("volume_id", str(bkp.get("volume_id", "") or "")),
        ("snapshot_id", str(bkp.get("snapshot_id", "") or "")),
        ("status", str(bkp.get("status", "") or "")),
        ("size", str(bkp.get("size", "") or "")),
        ("container", str(bkp.get("container", "") or "")),
        ("is_incremental", "yes" if bkp.get("is_incremental") else "no"),
        ("has_dependent_backups", "yes" if bkp.get("has_dependent_backups") else "no"),
        ("availability_zone", str(bkp.get("availability_zone", "") or "")),
        ("description", str(bkp.get("description", "") or "")),
        ("created_at", str(bkp.get("created_at", "") or "")),
        ("updated_at", str(bkp.get("updated_at", "") or "")),
    ]
    print_detail(fields, output_format=output_format, fit_width=fit_width,
                 max_width=max_width, noindent=noindent, columns=columns)
...
@click.option("--name", default=None, help="Backup name.")
@click.option("--description", default=None, help="Backup description.")
@click.option("--container", default=None, help="Optional backup container name.")
@click.option("--snapshot-id", default=None, help="Snapshot ID to backup instead of full volume.")
@click.option("--force", is_flag=True, default=False, help="Allow backing up an in-use volume.")
@click.option("--incremental", is_flag=True, default=False, help="Perform an incremental backup.")
@click.option("--wait", is_flag=True, default=False, help="Wait for backup to reach 'available'.")
@click.pass_context
def volume_backup_create(ctx: click.Context, volume_id: str, name: str | None,
                         description: str | None, container: str | None,
                         snapshot_id: str | None, force: bool, incremental: bool,
                         wait: bool) -> None:
    """Create a Cinder volume backup.

    \b
    Examples:
      orca volume backup-create <volume-id>
      orca volume backup-create <volume-id> --name my-backup --incremental
      orca volume backup-create <volume-id> --force --wait
    """
    client = ctx.find_object(OrcaContext).ensure_client()
    body: dict = {"volume_id": volume_id}
    if name is not None:
    ...
    if wait:
        wait_for_resource(
            client,
            f"{client.volume_url}/backups/{backup_id}",
            key="backup",
            target_status="available",
        )

    # Cinder POST /backups only returns id/name/links — fetch full details
    full = client.get(f"{client.volume_url}/backups/{backup_id}")
    bkp = full.get("backup", bkp)

    print_detail(
        [(k, str(bkp.get(k, "") or "")) for k in
         ("id", "name", "volume_id", "snapshot_id", "container",
          "description", "status", "size", "created_at")],
    )


@volume.command("backup-delete")
@click.argument("backup_id", callback=validate_id)
@click.option("--force", is_flag=True, default=False, help="Force delete even if backup is in error.")
@click.option("--yes", "-y", is_flag=True, help="Skip confirmation.")
@click.pass_context
def volume_backup_delete(ctx: click.Context, backup_id: str, force: bool, yes: bool) -> None:
    ...
    if force:
        params["force"] = True
    client.delete(f"{client.volume_url}/backups/{backup_id}", params=params)
    console.print(f"[green]Backup {backup_id} deleted.[/green]")


@volume.command("backup-restore")
@click.argument("backup_id", callback=validate_id)
@click.option("--volume-id", default=None, help="Restore to this existing volume ID.")
@click.option("--name", default=None, help="Name for the new volume (if not restoring to existing).")
@click.option("--wait", is_flag=True, default=False, help="Wait for restored volume to become 'available'.")
@click.pass_context
def volume_backup_restore(ctx: click.Context, backup_id: str,
                          volume_id: str | None, name: str | None, wait: bool) -> None:
    """Restore a Cinder volume backup.

    \b
    Examples:
      orca volume backup-restore <backup-id>
      orca volume backup-restore <backup-id> --name restored-vol
      orca volume backup-restore <backup-id> --volume-id <vol-id>
    """
    client = ctx.find_object(OrcaContext).ensure_client()
    body: dict = {}
    ...
    data = client.post(f"{client.volume_url}/backups/{backup_id}/restore",
                       json={"restore": body})
    restore = data.get("restore", data)

    if wait:
        restored_vol_id = restore.get("volume_id")
        if restored_vol_id:
            wait_for_resource(
                client,
                f"{client.volume_url}/volumes/{restored_vol_id}",
                key="volume",
                target_status="available",
            )

    print_detail(
        [(k, str(restore.get(k, "") or "")) for k in
         ("backup_id", "volume_id", "volume_name")],
    )
